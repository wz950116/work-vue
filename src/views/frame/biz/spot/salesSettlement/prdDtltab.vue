...
export default {
  data() {
    return {
      tab: {
        ...
        param: this.param,
        api: {
          search: '/api/contract/ssProductDetail/listBySettlementNo'
        },
        apiData: {
          search(param) {
        ...
        table: {
          cols: [
            ...
            {
              label: 'spot.contractLot.form.outTaxFreePrice',
              prop: 'outTaxFreePrice',
              width: '120',
              align: 'right',
              format: {
                func: 'money'
  ...
  methods: {
    ...
    changeSettlementTaxRate(data) {
      ...
      const row = data.row
      if (toolUtil.toNum(row.settlementTaxRate) === toolUtil.toNum(data.val)) {
        return
      }
      const settlementPrice = toolUtil.toNum(row.settlementPrice)
      const settlementTfprice = toolUtil.toNum(row.settlementTfprice)
    ...
    calcAmountAndSum(row) {
      ...
      // 刷新表格
      const prdDtlList = toolUtil.getDataTabFromSelf(this)
      prdDtlList.push({})
      prdDtlList.pop()
