...
export default {
  data() {
    return {
      dialog: {
        titleName: this.$t('spot.contractLot.popup.chooseLotInfoSl'),
        moreShowFlg: false,
        initType: 'param',
  ...
  methods: {
    getApi() {
      if (this.param.billType === 'dslc') {
        return '/api/domesticContract/slInfo/list'
      } else if (this.param.billType === 'eslc') {
        return '/api/externalContract/slInfo/list'
      } else {
        return '/api/contract/slInfo/list'
      }
    },
    getInitSearch() {
