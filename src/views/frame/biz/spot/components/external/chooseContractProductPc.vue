...
export default {
  data() {
    return {
      dialog: {
        titleName: this.$t('spot.contractLot.popup.chooseContractProductPc'),
        moreShowFlg: false,
        initType: 'param',
        ...
        mainData: {
          api: {
            search: '/api/contract/pcProductDetail/listChoose'
          },
          apiData: {
            search(param) {
  ...
  created() {
    ...
    this.dialog.mainData.table.cols.splice(7, 0, ...this.$store.state.user.mainGoodColsTxt)
    this.$store.dispatch('GoodsColListUpdate', 'CON_PC_INFO')
    this.dialog.mainData.table.cols.splice(this.$store.state.user.mainGoodColsTxt.length + 7, 0, ...this.$store.state.user.reserveGoodColsTxt)
  },
  mounted() {
        ...
        if (this.$props.initChooseParam.tradeTypeList.value.length > 0) {
          ...
          for (let i = 0; i < tradeTypeDict.length; i++) {
            if (this.$props.initChooseParam.tradeTypeList.value.includes(tradeTypeDict[i].value)) {
              ...
              if (tradeTypeDict[i].value === '1' || tradeTypeDict[i].value === '2') {
                tradeTypeDef.list.push(tradeTypeDict[i])
              }
            }
          }
