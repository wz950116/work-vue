        ...
        <el-table-column :key='col.prop' v-bind='col' :label='$t(col.label)' :sortable='!$t(col.isSon)' show-overflow-tooltip>
          ...
          <template slot-scope='scope'>
            ...
            <template v-else-if='col.isSon'>
              <div class='nest' style='display:block;height:24px;' v-for='(i, index) in scope.row["outProductList"]' :key="index" :style="{'border-top': index > 0 ? '1px solid #aaaaaa' : ''}">
...
export default {
  ...
  methods: {
    ...
    searchDataList(pkKeyList) {
      ...
      request({
        url: '/api/warehouse/whOutBill/list',
        method: 'post',
        data: {
          ...
          isPage: false,
          data: {
            outBillNoList: billNo,
            outProductNoList: billDtlNo,
            auditStatus: '3'
          }
        }
      }).then(response => {
