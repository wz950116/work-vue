        ...
        <el-table-column :key='col.prop' v-bind='col' :label='$t(col.label)' :sortable='!$t(col.isSon)' show-overflow-tooltip>
          ...
          <template slot-scope='scope'>
            ...
            <template v-else-if='col.isSon'>
              <div class='nest' style='display:block;height:24px;' v-for='(i, index) in scope.row["whLadingOutProductDtoList"]' :key="index" :style="{'border-top': index > 0 ? '1px solid #aaaaaa' : ''}">
    ...
    searchDataList(pkKeyList) {
      ...
      request({
        ...
        data: {
          ...
          data: {
            ladingOutNoList: billNo,
            ladingOutProductNoList: billDtlNo,
            auditStatus: '3'
          }
        }
      }).then(response => {
