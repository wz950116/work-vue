...
export default {
  name: 'domesticSalesLotChangeEdit',
  data() {
    return {
      edit: {
        ...
        api: {
          view: '/api/domesticContract/slcInfo/get',
          save: '/api/domesticContract/slcInfo/save',
          update: '/api/domesticContract/slcInfo/update'
        },
        apiData: {
          view(param) {
        ...
        topButtons: [
          ...
          },
          {
            ...
            isShow: ['add'],
            event: 'set',
            component: () => import('../../components/chooseLotInfoSl.vue'), // 组件
  ...
  methods: {
    ...
    loadFromLot(lotNo) {
      request({
        url: '/api/externalContract/slInfo/get',
        method: 'post',
        data: {
        ...
        if (response.data) {
          ...
          lotInfo.baseBillType = this.$route.meta.bizCode
          lotInfo.baseModuleCode = this.$route.meta.bpmModule
          lotInfo.headlineCodeList = param.headlineCodeList
          lotInfo.customerCodeList = param.customerCodeList
    ...
    closeLv2Dialog(param) {
      ...
      this.loadSubList(param.lotNo, productDetailList, '/api/contract/slProductDetail/listByLotNo')
      this.loadSubList(param.lotNo, paymentInfoList, '/api/contract/slPaymentInfo/listByLotNo')
      this.loadSubList(param.lotNo, feeList, '/api/contract/slEstimatedCost/listByLotNo')
      this.loadSubMarginList({ billType: 'DOM_CON_PC_INFO', billNo: param.contractNo }, marginRuleList, '/api/contract/conMarginRule/getByContractNo')
    },
    ...
    viewUpdate() {
      ...
      this.gotoPage('salesLotChangeUpdate', {
        type: 'update',
    ...
    viewRemove() {
      ...
      this.$confirm(this.$t('biz.msg.confirmDelete'), this.$t('biz.msg.tip'), {
        ...
        .then(() => {
          request({
            url: '/api/domesticContract/slcInfo/remove',
            method: 'post',
            data: {
              funcModule: this.$t('route.' + this.$route.meta.title),
              funcOperation: this.$t('biz.btn.search'),
    ...
    closePage(param) {
      const backName = this.$route.params.back ? this.$route.params.back : 'salesLotChange'
