...
export default {
  ...
  data() {
    return {
      edit: {
        ...
        api: {
          pre: {
            EXT_CON_SC_INFO: '/api/externalContract/scInfo/get'
          },
          view: '/api/externalContract/slInfo/get',
          save: '/api/externalContract/slInfo/save',
        ...
        topButtons: [
          ...
          },
          {
            ...
            isShow: ['add'],
            event: 'set',
            component: () => import('../../components/chooseContractProductSc.vue'), // 组件
            initChoose: this.contractSelectInitChoose,
  ...
  methods: {
    ...
    getUpdateApi(type) {
      switch (type) {
        case 'save':
          return '/api/externalContract/slInfo/save'
        case 'update':
          return '/api/externalContract/slInfo/update'
        case 'copy':
          return '/api/externalContract/slInfo/save'
      }
    },
    ...
    closeLv2Dialog(param) {
      const productList = this.$refs.qmEdit.$refs.productDetailList[0].$refs.tab.tableData
      ...
      const customerCodeList = this.getFormItemByProp('customerCodeList')
      ...
      customerCodeList.list.splice(0)
      ...
      for (let i = 0; i < contract.customerCodeList.length; i++) {
        customerCodeList.list.push({
          key: contract.customerCodeList[i],
          value: contract.customerCodeList[i],
          label: contract.customerNameList[i]
        })
      }
      ...
      for (const k in editForm) {
        if (k === 'optOrganCode' || k === 'optOrganName' || k === 'optDeptCode' || k === 'optDeptName' || k === 'optEmployeeCode' || k === 'optEmployeeName' || k === 'bizOrganCode' || k === 'bizOrganName' || k === 'bizDeptCode' || k === 'bizDeptName' || k === 'bizEmployeeCode' || k === 'bizEmployeeName' || k === 'auditStatus' || k === 'remark') {
          continue
        }
      ...
      this.loadSubMarginList({ billType: 'EXT_CON_SC_INFO', billNo: contract.contractNo }, marginRuleList, '/api/contract/conMarginRule/getByContractNo')

      editForm.deliveryModeList = contract.deliveryMode.split(',')
      editForm.baseBillType = this.$route.meta.bizCode
      editForm.baseModuleCode = this.$route.meta.bpmModule
    ...
    viewUpdate() {
      ...
      this.gotoPage('extSalesLotUpdate', {
        type: 'update',
    ...
    viewRemove() {
      ...
      this.$confirm(this.$t('biz.msg.confirmDelete'), this.$t('biz.msg.tip'), {
        ...
        .then(() => {
          request({
            url: '/api/externalContract/slInfo/remove',
            method: 'post',
            data: {
              funcModule: this.$t('route.' + this.$route.meta.title),
              funcOperation: this.$t('biz.btn.search'),
    ...
    openModifyAdd() {
      ...
      request({
        url: '/api/externalContract/slInfo/checkChangeAble',
        method: 'post',
        data: {
          funcModule: this.$t('route.' + this.$route.meta.title),
          funcOperation: this.$t('biz.btn.search'),
        ...
        if (response.status) {
          this.openPage('extSalesLotChangeAdd', {
            type: 'add',
            lotNo: editForm.lotNo
          })
        }
    ...
    viewArchive() {
      ...
      this.$confirm(this.$t('spot.contractLot.msg.closeLotConfirmed'), this.$t('notify.title.warning'), {
        ...
        request({
          url: '/api/externalContract/slInfo/achive',
          method: 'post',
          data: {
            funcModule: this.$t('route.' + this.$route.meta.title),
            funcOperation: this.$t('biz.btn.search'),
    ...
    goodFinish() {
      ...
      this.$confirm(this.$t('spot.contractLot.msg.lotGoodFinishConfirmed'), this.$t('notify.title.warning'), {
        ...
        request({
          url: '/api/externalContract/slInfo/goodFinish',
          method: 'post',
          data: {
            funcModule: this.$t('route.' + this.$route.meta.title),
            funcOperation: this.$t('biz.btn.search'),
    ...
    fundFinish() {
      ...
      this.$confirm(this.$t('spot.contractLot.msg.lotFundFinishConfirmed'), this.$t('notify.title.warning'), {
        ...
        request({
          url: '/api/externalContract/slInfo/fundFinish',
          method: 'post',
          data: {
            funcModule: this.$t('route.' + this.$route.meta.title),
            funcOperation: this.$t('biz.btn.search'),
    ...
    invoFinish() {
      ...
      this.$confirm(this.$t('spot.contractLot.msg.lotInvoFinishConfirmed'), this.$t('notify.title.warning'), {
        ...
        request({
          url: '/api/externalContract/slInfo/invoFinish',
          method: 'post',
          data: {
            funcModule: this.$t('route.' + this.$route.meta.title),
            funcOperation: this.$t('biz.btn.search'),
    ...
    doHistory() {
      const editForm = toolUtil.getDataEditFromSelf(this)
      this.openPage('extSalesLotHistory', {
        lotNo: editForm.lotNo
      })
    },
    archiveShow() {
    ...
    initCallback(data) {
      ...
      if (this.edit.param.type === 'update') {
        request({
          url: '/api/contract/scHeadline/listByContractNo',
          method: 'post',
          data: {
            funcModule: this.$t('route.' + this.$route.meta.title),
            funcOperation: this.$t('biz.btn.search'),
        ...
        request({
          url: '/api/contract/scCustomer/listByContractNo',
          method: 'post',
          data: {
            funcModule: this.$t('route.' + this.$route.meta.title),
            funcOperation: this.$t('biz.btn.search'),
          ...
          if (response.status) {
            for (let i = 0; i < response.data.length; i++) {
              customerCodeList.list.push({
                key: response.data[i].customerCode,
                value: response.data[i].customerCode,
                label: response.data[i].customerName
              })
            }
          }
        })
        ...
        for (let i = 0; i < data.customerCodeList.length; i++) {
          customerCodeList.list.push({
            key: data.customerCodeList[i],
            value: data.customerCodeList[i],
            label: data.customerNameList[i]
          })
        }
      }
    ...
    closePage(param) {
      const backName = this.$route.params.back ? this.$route.params.back : 'extSalesLot'
    ...
    handlePreCallback(contract) {
      ...
      request({
        url: '/api/contract/scProductDetail/listByContractNo',
        method: 'post',
        data: {
          funcModule: this.$t('route.' + this.$route.meta.title),
          funcOperation: this.$t('biz.btn.search'),
      ...
      request({
        url: '/api/contract/scPaymentInfo/listByContractNo',
        method: 'post',
        data: {
          funcModule: this.$t('route.' + this.$route.meta.title),
          funcOperation: this.$t('biz.btn.search'),
