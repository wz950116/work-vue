...
export default {
  ...
  methods: {
    ...
    dispChartCst(productInfo) {
      ...
      request({
        url: '/api/contract/scReport/reportByCustomer',
        method: 'POST',
        data: {
          data: {
          ...
          axisX.push({
            value: {
              ...
              toString() {
                ...
                } else if (this.customerCode !== undefined && this.customerCode !== null && this.customerCode !== '') {
                  return this.customerCode.toString()
                }
                return ''
              }
            }
        ...
        const chartCst = this.$echarts.init(this.$refs.chartCst)
        chartCst.setOption({
          title: { text: '客户业务量统计表(' + productName + ')' },
          tooltip: {
            trigger: 'axis',
            axisPointer: {
              type: 'shadow'
          ...
          legend: {
            data: ['销售额(万元)', '发票额(万元)', '销售量(' + measurementUnitName + ')']
          },
          series: [
            {
              name: '销售额(万元)',
