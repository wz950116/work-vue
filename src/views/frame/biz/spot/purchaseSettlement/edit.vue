...
export default {
  ...
  data() {
    return {
      edit: {
        ...
        api: {
          view: '/api/contract/psInfo/get',
          save: '/api/contract/psInfo/save',
          update: '/api/contract/psInfo/update'
        },
        apiData: {
          view(param) {
  ...
  methods: {
    ...
    handleCloseDialog(param) {
      this.$store.dispatch('delVisitedViews', this.$route).then(() => this.$router.push({ name: 'purchaseSettlement' }))
    },
    closeLv2Dialog(param) {
      ...
      for (let i = 1; i < param.length; i++) {
        ...
        if (param[i].supplierCode !== firstRow.supplierCode) {
          this.$notify(notifyInfo({ msg: this.$t('spot.contractLot.msg.chooseSettlementProduct3') }))
          return
        }
      }
      ...
      for (let i = 0; i < param.length; i++) {
        const inBillInfo = param[i]
        let newRow = null
        for (let j = 0; j < prdDtlList.length; j++) {
          const prdRow = prdDtlList[j]
          if (inBillInfo.productCode === prdRow.productCode && inBillInfo.brand === prdRow.brand && inBillInfo.spec === prdRow.spec && inBillInfo.origin === prdRow.origin && inBillInfo.productKeyAttr01 === prdRow.productKeyAttr01 && inBillInfo.productKeyAttr02 === prdRow.productKeyAttr02 && inBillInfo.productKeyAttr03 === prdRow.productKeyAttr03 && inBillInfo.inPrice === prdRow.inPrice && inBillInfo.inTaxRate === prdRow.inTaxRate) {
            newRow = prdRow
            break
          }
        }
        if (newRow === null) {
          newRow = Object.assign({}, inBillInfo)
          newRow.inProductNo = inBillInfo.inProductNo
          newRow.settlementPrice = inBillInfo.inPrice
          newRow.settlementTfprice = inBillInfo.inTaxFreePrice
          newRow.settlementQuantity = inBillInfo.settlementQuantity
          newRow.settlementTaxRate = inBillInfo.inTaxRate
          newRow.inBillList = []
          prdDtlList.push(newRow)
        } else {
          newRow.inQuantity = toolUtil.toNum(newRow.inQuantity) + toolUtil.toNum(inBillInfo.inQuantity)
          newRow.acceptQuantity = toolUtil.toNum(newRow.acceptQuantity) + toolUtil.toNum(inBillInfo.acceptQuantity)
          newRow.returnQuantity = toolUtil.toNum(newRow.returnQuantity) + toolUtil.toNum(inBillInfo.returnQuantity)
          newRow.settlementQuantity = toolUtil.toNum(newRow.settlementQuantity) + toolUtil.toNum(inBillInfo.settlementQuantity)
        }
        newRow.inBillList.push({
          inBillType: inBillInfo.inBillType,
          inBillNo: inBillInfo.inBillNo,
          inProductNo: inBillInfo.inProductNo
        })
      }

      editForm.headlineCode = this.defaultNull(firstRow.headlineCode)
      editForm.headlineName = this.defaultNull(firstRow.headlineName)
      editForm.supplierCode = this.defaultNull(firstRow.supplierCode)
      editForm.supplierName = this.defaultNull(firstRow.supplierName)
      ...
      for (let i = 0; i < prdDtlList.length; i++) {
        inBillKeyList.push({
          billType: prdDtlList[i].inBillType,
          billNo: prdDtlList[i].inBillNo,
          billDtlNo: prdDtlList[i].inProductNo
        })
      }
      request({
        url: '/api/contract/psFeeDetail/listByInBill',
        method: 'post',
        data: {
    ...
    viewUpdate() {
      ...
      this.$store.dispatch('delVisitedViews', this.$route).then(() => {
        this.$router.push({
          name: 'purchaseSettlementUpdate',
          params: {
            type: 'update',
            back: 'purchaseSettlement',
    ...
    viewRemove() {
      ...
      this.$confirm(this.$t('biz.msg.confirmDelete'), this.$t('biz.msg.tip'), {
        ...
        .then(() => {
          request({
            url: '/api/contract/psInfo/remove',
            method: 'post',
            data: {
              funcModule: this.$t('route.' + this.$route.meta.title),
              funcOperation: this.$t('biz.btn.search'),
      ...
      if (editForm.settlementType === '1') {
        ...
        for (let i = 0; i < feeDtlList.length; i++) {
          ...
          for (let j = 0; j < sttDtlList.length; j++) {
            ...
            let matchFlag = false
            for (let k = 0; k < row.inBillList.length; k++) {
              if (row.inBillList[i].inBillType === feeRow.inBillType && row.inBillList[i].inBillNo === feeRow.inBillNo && row.inBillList[i].inProductNo === feeRow.inProductNo) {
                matchFlag = true
                break
              }
            }
