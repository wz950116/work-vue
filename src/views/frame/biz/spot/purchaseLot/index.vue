    ...
    <template v-if="true||route.name === $options.name || ( $route.meta && $route.meta.componentName === $options.name)">
      <qm-table-array ref="qmTable" :mainData="mainData">
        <template slot='contractNo' slot-scope='scope'>
          <router-link :to="{name: 'purchaseContractView', params: {type: 'view',back: 'purchaseLot','contractNo':scope.row.contractNo }}">{{ scope.row.contractNo }}</router-link>
        </template>
      </qm-table-array>
    </template>
  </div>
