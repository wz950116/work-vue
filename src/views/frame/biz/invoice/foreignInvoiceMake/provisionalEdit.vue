...
export default {
    ...
    return {
      edit: {
        ...
        api: {
          ...
          pre: {
            CON_SL_INFO: '/api/contract/slProductDetail/listChooseInvoiceF'
          }
        },
        apiData: {
          pre(param) {
        ...
        topButtons: [
          ...
          },
          {
            ...
            component: () => import('@/views/frame/biz/spot/dialog/invoice/chooseLotProductSF.vue'),
        ...
        formData: {
          part1: {
            titleName: 'invoice.foreignInvoiceMake.group.provisionalEditPart1',
            content: [
              {
  ...
  methods: {
    handlePreCallback(result) {
      if (result.length === 0) {
        this.$notify(notifyError({ msg: this.$t('invoice.foreignInvoiceMake.msg.noPostData') }))
        return
      }
    ...
    selectCallback(params) {
      if (params) {
        ...
        editForm.customerNo = lotInfo.customerCodeList[0]
        editForm.customerName = lotInfo.customerNameList[0]
        ...
        this.resetOptions(params[0].lot)
      }
    },
    ...
    async resetOptions(param) {
      const supplierList = []
      if (param) {
        param.customerCodeList.forEach((s, index) => {
          supplierList.push({
            code: s,
            name: param.customerNameList[index]
          })
        })
      } else {
