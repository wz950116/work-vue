...
export default {
  data() {
    return {
      tab: {
        ...
        api: {
          search: '/api/fee/invoiceAllotDetail/listByAllotNo'
        },
        apiData: {
          search(param) {
        ...
        table: {
          ...
          cols: [
            // 合同号
            {
              label: 'fee.feeInvoiceAllot.list.contractNo',
              prop: 'contractNo',
              width: 200,
              align: 'left'
            },
            // 批次号
            {
              label: 'fee.feeInvoiceAllot.list.lotNo',
              prop: 'lotNo',
              width: 200,
              align: 'left'
            },
            ...
            // 分摊金额
            {
              label: 'fee.feeInvoiceAllot.list.allotAmount',
              prop: 'allotAmount',
              element: 'input-formatter',
              attrs: {
                type: 'money',
  ...
  methods: {
    ...
    allotQuantityChange(val) {
      // 赋值分摊金额
      val.row.allotAmount =
        Number(val.row.allotQuantity) * Number(this.editForm.includeTaxPrice)
    },
    editChange() {
      // 赋值基本信息里的分摊数量
      var allotQuantity = 0
      ...
      this.editForm.excludeTaxAmount = (
        Number(allotAmount) /
        (1 + Number(this.editForm.taxRate) / 100)
      ).toFixed(2)
      this.editForm.taxAmount = (
        Number(this.editForm.includeTaxAmount) -
        Number(this.editForm.excludeTaxAmount)
      ).toFixed(2)
    },
    allotAmountChange(value) {
      // 赋值分摊金额
      value.row.allotQuantity =
        Number(value.val) / Number(this.editForm.includeTaxPrice)
      this.editChange()
    }
  }
}
