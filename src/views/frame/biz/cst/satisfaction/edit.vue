<template>
  <div class='app-container edit-page-style'>
    ...
    <!-- 主信息 -->
    <header>
      <div class='form-title'>{{$t('cst.satisfaction.template.title')}}</div>
      <el-form :inline="true" :model="formData" class="header-form-inline" label-position="left" ref="validateForm">
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item :label="$t('cst.satisfaction.template.templateName')" prop='templateName'>
              <el-input v-model="formData.templateName" :readonly="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="$t('cst.satisfaction.template.templateCode')" prop='templateCode'>
              <el-input v-model="formData.templateCode" :readonly="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="$t('cst.satisfaction.template.custName')" prop='custName'>
              <el-input v-model="formData.custName" :readonly="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="$t('cst.satisfaction.template.typeName')" prop='typeName'>
              <el-input v-model="formData.typeName" :readonly="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="$t('cst.satisfaction.template.businessType')" prop='businessType'>
              <el-input v-model="formData.businessType" :readonly="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="$t('cst.satisfaction.template.startDate')" prop='startDate'>
              <el-date-picker v-model="formData.startDate" :readonly="true" type="date" format="yyyy-MM-dd" value-format='yyyyMMdd' />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="$t('cst.satisfaction.template.sellAmount')" prop='sellAmount'>
              <input-formatter v-model="formData.sellAmount" type="money" :readonly="true"></input-formatter>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="$t('cst.satisfaction.template.productType')" prop='productType'>
    ...
    <header v-for="column in questionList" :key="column.name">
      ...
      <el-table stripe border :height="column.questionList.length * 25 + 27" highlight-current-row :data="column.questionList" class='table-content tb-edit' style="border:1px solid;">
        ...
        <el-table-column :label='$t("cst.satisfaction.template.question")' width="200">
          <template slot-scope="scope">
            {{scope.row.question}}
          </template>
        </el-table-column>
        <el-table-column :label='$t("cst.satisfaction.template.point")' width="120">
          <template slot-scope="scope">
    ...
    <!-- 备注 -->
    <header>
      <div class='form-title'>{{$t('cst.satisfaction.template.remark')}}</div>
...
export default {
  ...
  methods: {
    getData() {
      request({
        url: '/api/cust/custSatisfaction/get',
        method: 'POST',
        data: {
    ...
    initData() {
      request({
        url: '/api/cust/custSatisfaction/init',
        method: 'POST',
        data: {
          funcModule: this.$t('route.' + this.$route.meta.title),
          funcOperation: this.$t('biz.btn.save'),
    ...
    initQuestion() {
      request({
        ...
        var satisfactionColumn = {}
        for (let i = 0; i < this.$t('datadict.satisfactionColumn').length; i++) {
          const item = this.$t('datadict.satisfactionColumn')[i]
          satisfactionColumn[item.value] = item.label
        }
        ...
        for (var i = 0; i < response.data.length; i++) {
          ...
          const columnName = satisfactionColumn[item.templateColumn] // 栏目名称
    ...
    getAnswers() {
      request({
        url: '/api/cust/custSatisfactionDetail/get',
        method: 'POST',
        data: {
          funcModule: this.$t('route.' + this.$route.meta.title),
          funcOperation: this.$t('biz.btn.save'),
    ...
    save() {
      ...
      var url = '/api/cust/custSatisfaction/update'
      if (this.type === 'add') {
        url = '/api/cust/custSatisfaction/save'
      }
