  ...
  data() {
    return {
      tab: {
        ...
        api: {
          search: '/api/warehouse/whPnlProduct/listByPnlNo'
        },
        apiData: {
          search(param) {
        ...
        table: {
          cols: [
            ...
            // 库区
            {
              width: 80,
              align: 'left',
              label: 'storage.profitAndLoss.list.stockArea',
              prop: 'stockArea'
            },
            // 库位
            {
              width: 80,
              align: 'left',
              label: 'storage.profitAndLoss.list.stockPosition',
              prop: 'stockPosition'
            },
            // 仓储号
            {
              width: 80,
              align: 'left',
              label: 'storage.profitAndLoss.list.stockNo',
              prop: 'stockNo'
            },
  ...
  methods: {
    ...
    handleCloseDialog(params) {
      ...
      params.forEach((row, index) => {
        ...
        if (Number(row.restQuantity) > 0) {
          this.$refs.tab.tableData[_index]['pnlType'] = 'LOSS'
        }
        if (Number(row.restQuantity) < 0) {
          this.$refs.tab.tableData[_index]['pnlType'] = 'PROFIT'
        }
        this.$refs.tab.tableData[_index]['itemCount'] = '0'
        this.$refs.tab.tableData[_index]['pnlQuantity'] = Math.abs(row.restQuantity)
        this.$refs.tab.tableData[_index]['measurementQuantity'] = Math.abs(row.restMeasurementQuantity)
