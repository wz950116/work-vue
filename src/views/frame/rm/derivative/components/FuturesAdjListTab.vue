...
<template>
  <div>
    ...
    <template v-if='adjustTypeChange=== "2"'>
      ...
      <el-table v-if="this.type==='view'" :height="tableHeight" :data="myDetailData" key="supDetailData" stripe v-loading="loading" :element-loading-text="$t('route.load')" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.1)" border class='table-content tb-edit' ref="multipleTable" style="border:1px solid">
        ...
        <!-- 补录只显示开盘价的 v-if="!disabled"-->
        <el-table-column align="right" prop='openPrice' :label="$t('quotation.futuresPriceAdjust.openPrice')" min-width='200'></el-table-column>
        <el-table-column align="right" prop='closingPrice' :label="$t('quotation.futuresPriceAdjust.closingPrice')" min-width='200'></el-table-column>
        <el-table-column align="right" prop='highestPrice' :label="$t('quotation.futuresPriceAdjust.highestPrice')" min-width='200'></el-table-column>
        <el-table-column align="right" prop='lowestPrice' :label="$t('quotation.futuresPriceAdjust.lowestPrice')" min-width='200'></el-table-column>
        <el-table-column align="right" prop='settlementPrice' :label="$t('quotation.futuresPriceAdjust.settlementPrice')" min-width='200'></el-table-column>
        <el-table-column align="center" prop='currencyName' :label="$t('quotation.futuresPriceAdjust.currency')" min-width='100'>
        </el-table-column>
        <el-table-column prop='result' :label="$t('quotation.futuresPriceAdjust.result')" min-width='220'></el-table-column>
      </el-table>
      <el-table v-else :height="tableHeight" :data="myDetailData" key="supDetailData" stripe v-loading="loading" :element-loading-text="$t('route.load')" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.1)" border class='table-content tb-edit' ref="multipleTable" style="border:1px solid">
        <el-table-column align='center' type="index" width="50" label=' ' fixed></el-table-column>
        <el-table-column align='center' prop='priceDate' :label="$t('quotation.futuresPriceAdjust.priceDate')" width='100'>
          <template slot-scope='scope'>
            <span>{{scope.row.priceDate | dateFormat}}</span>
        ...
        <el-table-column align="right" prop='openPriceOld' :label="$t('quotation.futuresPriceAdjust.openPriceOld')" min-width='200' v-if="disabled">
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.openPriceOld | numFilter(2) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.openPrice' size='mini'></input-formatter>
          </template>
        </el-table-column>
        ...
        <el-table-column align="right" prop='closingPriceOld' :label="$t('quotation.futuresPriceAdjust.closingPriceOld')" min-width='200' v-if="disabled">
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.closingPriceOld | numFilter(2) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.closingPrice' size='mini'></input-formatter>
          </template>
        </el-table-column>
        ...
        <el-table-column align="right" prop='highestPriceOld' :label="$t('quotation.futuresPriceAdjust.highestPriceOld')" min-width='200' v-if="disabled">
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.highestPriceOld | numFilter(2) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.highestPrice' size='mini'></input-formatter>
          </template>
        </el-table-column>
        ...
        <el-table-column align="right" prop='lowestPriceOld' :label="$t('quotation.futuresPriceAdjust.lowestPriceOld')" min-width='200' v-if="disabled">
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.lowestPriceOld | numFilter(2) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.lowestPrice' size='mini'></input-formatter>
          </template>
        </el-table-column>
        ...
        <el-table-column align="right" prop='settlementPriceOld' :label="$t('quotation.futuresPriceAdjust.settlementPriceOld')" min-width='200' v-if="disabled">
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.settlementPriceOld | numFilter(2) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.settlementPrice' size='mini'></input-formatter>
          </template>
        </el-table-column>
    ...
    <template v-else>
      ...
      <el-table :height="tableHeight" :data="myDetailData" key="adjDetailData" stripe v-loading="loading" :element-loading-text="$t('route.load')" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.1)" border class='table-content tb-edit' ref="multipleTable" style="border:1px solid" @row-click="handleCurrent" @selection-change="selectionChange">
        ...
        <el-table-column align='right' prop='openPrice' :label="$t('quotation.futuresPriceAdjust.openPrice')" min-width='200'>
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.openPrice | numFilter(8) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.openPrice' size='mini' :disabled="type==='view'"></input-formatter>
          </template>
        </el-table-column>
        ...
        <el-table-column align='right' prop='closingPrice' :label="$t('quotation.futuresPriceAdjust.closingPrice')" min-width='200'>
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.closingPrice | numFilter(8) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.closingPrice' size='mini' :disabled="type==='view'"></input-formatter>
          </template>
        </el-table-column>
        ...
        <el-table-column align="right" prop='highestPrice' :label="$t('quotation.futuresPriceAdjust.highestPrice')" min-width='200'>
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.highestPrice | numFilter(8) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.highestPrice' size='mini' :disabled="type==='view'"></input-formatter>
          </template>
        </el-table-column>
        ...
        <el-table-column align="right" prop='lowestPrice' :label="$t('quotation.futuresPriceAdjust.lowestPrice')" min-width='200'>
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.lowestPrice | numFilter(8) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.lowestPrice' size='mini' :disabled="type==='view'"></input-formatter>
          </template>
        </el-table-column>
        ...
        <el-table-column align='right' prop='settlementPrice' :label="$t('quotation.futuresPriceAdjust.settlementPrice')" min-width='200'>
          <template slot-scope='scope'>
            <span v-show="disabled">{{scope.row.settlementPrice | numFilter(8) | thousandth}}</span>
            <input-formatter v-show="!disabled" :max='999999999999.9999' :min='0' :precision='8' :controls='false' type='thousands' v-model='scope.row.settlementPrice' size='mini' :disabled="type==='view'"></input-formatter>
          </template>
        </el-table-column>
    ...
    <!-- 弹窗 -->
    <edit-dialog v-if='dialogVisible' @close='closeHandlerDialog' :myDetailData='myDetailData'></edit-dialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    tableHeight() {
      return this.clientWidth > 1355 ? this.clientHeight - 370 : this.clientHeight - 390 // 防止底部滚动条遮挡
    }
  },
  filters: {
  ...
  methods: {
          ...
          results.forEach(element => {
            ...
            getFuturesVarietiesName(
              ...
              .then(response => {
                ...
                if (futuresVarietiesNameList.indexOf(obj.futuresVarietiesName) === -1) {
                  // obj.tempResult = '品种不存在,请确认!'
                  // obj.result.push(obj.tempResult)
                  obj.result += '品种不存在,请确认!'
                  this.pageLoading = false
                }
              })
              .catch(() => {
            ...
            getFuturesContractCode(
              ...
              .then(response => {
                ...
                if (futuresContractCodeList.indexOf(obj.futuresContractCode) === -1) {
                  obj.result += '合约代码不存在,请确认!'
                  this.pageLoading = false
                }
                if (futuresContractNameList.indexOf(obj.futuresContractName) === -1) {
                  obj.result += '合约名称不存在,请确认!'
                  this.pageLoading = false
                }
              })
              .catch(() => {
            ...
            var re = /^[0-9]+.?[0-9]*$/
            // 价格都为空,则跳过这个数据继续执行
            if ((obj.openPrice === undefined || obj.openPrice === '') && (obj.closingPrice === undefined || obj.closingPrice === '') && (obj.highestPrice === undefined || obj.highestPrice === '') && (obj.lowestPrice === undefined || obj.lowestPrice === '') && (obj.settlementPrice === undefined || obj.settlementPrice === '')) {
              return
            } else {
              // 开盘价
              if (!re.test(obj.openPrice) || obj.openPrice === '' || obj.openPrice === undefined) {
                obj.result += '开盘价只能输入数字或不能为空!'
                this.pageLoading = false
              }
              // 收盘价
              if (!re.test(obj.closingPrice) || obj.closingPrice === '' || obj.closingPrice === undefined) {
                obj.result += '收盘价只能输入数字或不能为空!'
                this.pageLoading = false
              }
              // 最高价
              if (!re.test(obj.highestPrice) || obj.highestPrice === '' || obj.highestPrice === undefined) {
                obj.result += '最高价只能输入数字或不能为空!'
                this.pageLoading = false
              }
              // 最低价
              if (!re.test(obj.lowestPrice) || obj.lowestPrice === '' || obj.lowestPrice === undefined) {
                obj.result += '最低价只能输入数字或不能为空!'
                this.pageLoading = false
              }
              // 结算价
              if (!re.test(obj.settlementPrice) || obj.settlementPrice === '' || obj.settlementPrice === undefined) {
                obj.result += '结算价只能输入数字或不能为空!'
                this.pageLoading = false
              }
            }
            // 合约代码非空校验
    ...
    downTemp() {
      ...
      futuresPriceAdjust.listFutureAdjustWait({ data: date }, this.func).then(response => {
        import('@/vendor/frame/Export2Excel').then(excel => {
          const tHeader = ['价格日期', '交易所', '品种', '合约名称', '合约代码', '开盘价', '收盘价', '最高价', '最低价', '结算价', '币种']
          const filter = ['priceDate', 'exchangeName', 'futuresVarietiesName', 'futuresContractName', 'futuresContractCode', 'openPrice', 'closingPrice', 'highestPrice', 'lowestPrice', 'settlementPrice', 'currencyName']
