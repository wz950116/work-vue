<template>
  <el-select v-model="checkItem" @change="change" :clearable="clearable" :filterable="filterable" allow-create default-first-option :disabled="disabled" :size="size" :multiple="multiple" collapse-tags>
    <el-option v-for="item in listData" :key="item.id" :label="item.customerName" :value="item.derivativeAccount"></el-option>
  </el-select>
</template>

<script>
import { getList } from '@/api/frame/rm/derivative/optionAccountTrade'
