        ...
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item :label="$t('strategy.strategyManagement.form.projectCode')" prop='projectCode'>
              <base-select v-model="formData.projectCode" :attrs="{data: 'PROJECT',clearable: true,params:{auditStatus:'3', usingFlag:'1'},disabled:true}"></base-select>
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item :label="$t('strategy.strategyManagement.form.makeChangeDate')" prop='makeDate'>
...
import hedgingPortfolio from '@/views/frame/rm/strategy/strategyChangeManagement/hedging/hedgingPortfolio'
    ...
    checkDetail() {
      ...
      list.forEach(element => {
        ...
        if (
          element.itemClass === '2' &&
          !element.maxHands &&
          !errMsg.includes('类型为期货时,最大开仓手数不能为空')
        ) {
        ...
        if (
          element.itemClass === '1' &&
          !element.maxPostion &&
          !errMsg.includes('类型为现货时,最大开仓量不能为空')
        ) {
