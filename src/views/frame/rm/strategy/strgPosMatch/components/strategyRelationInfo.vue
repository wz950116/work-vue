  ...
  <div>
    <div class='tabs-operate' style='margin-top:5px;'>
      <el-button v-db-click size="mini" @click="addRow" v-if="['add', 'update'].includes(type)">
        <svg-icon icon-class="线性-增行"></svg-icon>{{ $t('biz.btn.selectStrategy') }}
      </el-button>
    </div>
    ...
    <strategyRelationDialog v-if="dialogVisible" :contractCode='contractCode' :direction='direction' :showZero="showZero" @close="dialogClose" @confirm="confirm"></strategyRelationDialog>
  </div>
</template>

<script>
import strategyRelationDialog from '@/views/frame/rm/strategy/strgPosMatch/components/strategyRelationDialog'
import { notifyInfo } from '@/utils/frame/base/notifyParams'

export default {
  components: {
    strategyRelationDialog
  },
  data() {
    return {
  ...
  methods: {
    addRow() {
      if (this.$parent.$parent.$parent.$refs.strategyInfo.$refs.strategyInfo1.tableData.length === 0) {
        this.$notify(
          notifyInfo({
            msg: this.$t('strategy.posMatch.msg.strgDetailEmpty')
          })
        )
        return
      }
      if (this.$parent.$parent.$parent.$refs.tradeInfo.tableData.length === 0) {
        this.$notify(
          notifyInfo({
            msg: this.$t('strategy.posMatch.msg.tradeDataEmpty')
          })
        )
        return
      }
      this.contractCode = this.$parent.$parent.$parent.$refs.tradeInfo.tableData[0].futuresContractCode
      this.direction = this.$parent.$parent.$parent.$refs.tradeInfo.tableData[0].direction
      let offsetFlag = this.$parent.$parent.$parent.$refs.tradeInfo.tableData[0].offsetFlag
      if (this.direction === '1') {
        this.direction = '2'
      } else {
        this.direction = '1'
