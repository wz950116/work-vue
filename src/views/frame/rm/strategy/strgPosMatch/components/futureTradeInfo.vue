<template>
  <div>
    ...
    <futureTradeDialog v-if="dialogVisible" :tradeNature="'3'" :productCode='productCode' @close="dialogClose" @confirm="confirm"></futureTradeDialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addRow() {
      if (
        this.$parent.$parent.$parent.$refs.strategyInfo.$refs.strategyInfo1.tableData.length ===
        0
      ) {
        this.$notify(
          notifyInfo({
      ...
      this.direction = this.$parent.$parent.$parent.$refs.strategyInfo.$refs.strategyInfo1.tableData[0].direction
