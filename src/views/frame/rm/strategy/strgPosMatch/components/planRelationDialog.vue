    ...
    <div class='dialog-container' :style="{width: this.sidebar.opened ? '1000px' : '1150px'}">
      ...
      <header style='border-radius:5px;'>
        <el-form ref='queryForm' label-position="left" :inline="true" :model="listQuery.data" class='header-form-inline'>
          <el-row :gutter="20" style='width:94%;'>
            <el-col :span="8">
              <el-form-item :label="$t('strategy.posMatch.list.planCode')" prop="planCode">
                <input-validate v-model="listQuery.data.planCode" :clearable='true'></input-validate>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item :label="$t('strategy.posMatch.list.planName')" prop='planName'>
      ...
      <main>
        <el-tabs v-model="activeNameBottom" type="border-card">
          <el-tab-pane :label="$t('strategy.posMatch.tab.planRelation')" name="exposureInfo">
...
export default {
  ...
  computed: {
    ...
    titleName() {
      return this.$t('strategy.posMatch.tab.planRelation')
    }
  },
  props: {
    contractCode: {
