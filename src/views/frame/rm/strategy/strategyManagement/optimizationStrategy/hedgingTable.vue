    ...
    <el-table stripe border height="150" highlight-current-row ref="sellTable" class='table-content tb-edit' :data="sellData" style="border:1px solid;" @selection-change="sellHandleSelectionChange">
      ...
      <el-table-column :label="$t('strategy.strategyManagement.tab.itemClass')" width="100">
        ...
        <template slot-scope="scope">
          <el-select @change='v=>changeSell({val:v,row:scope.row})' size='mini' v-model="scope.row.itemClass" :placeholder="$t('biz.placeholder.choose')" :clearable="true">
            <el-option v-for="item in $t('datadict.itemClass')" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('strategy.strategyManagement.tab.maxHands')" width="120">
        ...
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.maxHands" @keyup.native="number" @change='v=>changeMaxHands({val:v,row:scope.row})' :disabled="scope.row.itemClass==='1'"></el-input>
        </template>
      </el-table-column>
