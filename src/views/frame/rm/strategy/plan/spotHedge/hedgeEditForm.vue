<template>
  <div class='app-container edit-page-style'>
    ...
    <el-row class='top-operate'>
      <el-button v-db-click style='margin-right:6px;' size="mini" type="primary" @click="close" v-if="['view', 'changeView'].includes(type)">
      ...
      <el-button v-db-click style='margin-right:6px;' size="mini" type="primary" @click="save" v-if="['add', 'update','change','changeUpdate'].includes(type)">
        <svg-icon icon-class="线性-保存"></svg-icon>{{$t('biz.btn.save')}}
      </el-button>
      <el-button v-db-click style='margin-right:6px;' size="mini" type="primary" @click="audit" v-if="['assist', 'audit'].includes(type)">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item :label="$t('strategy.plan.form.applicant')" prop='applicant'>
              <base-select v-model="formData.applicant" @change="applicantChange" :disabled="matchFlag" :clearable='true' :attrs="{data: 'TREE_STAFF'}"></base-select>
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item :label="$t('strategy.plan.form.maxShare')" prop='maxShare'>
              <input-formatter v-model="formData.maxShare" :min="0" :precision="0" :disabled="matchFlag" :max="formData.strgMaxShare || 999999999"></input-formatter>
            </el-form-item>
          </el-col>
          <el-col :span="6">
    ...
    <main>
      ...
      <el-tabs v-if="changeVisible" v-model="bottomName" type="border-card">
        <el-tab-pane v-if="changeVisible" :label="$t('strategy.plan.tab.changeHistory')" name="changeHistory">
...
<script>
import hedgingPortfolio from '@/views/frame/rm/strategy/plan/spotHedge/hedgingPortfolio'
...
export default {
  ...
  mounted() {
    ...
    this.change || this.type.includes('change') ? 'planChange' : 'planManage'
  ...
  methods: {
    ...
    checkSave() {
      ...
      for (var i = 0; i < this.formData.detailList.length; i++) {
        ...
        if (this.formData.enterMode === '2' && (!data.maxPrice || !data.minPrice)) {
          this.$notify(
            notifyError({
