    ...
    <header :style="{'border-bottom-width': mainInfoVisible ? '1px' : '0'}">
      ...
      <el-form v-if='mainInfoVisible' :inline="true" :model="formData" class="header-form-inline" label-position="left" :rules='rules' ref="validateForm">
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item :label="$t('strategy.plan.form.strategyCode')" prop='strategyCode'>
    ...
    <main>
      <el-tabs v-model="activeName" type="border-card">
        <el-tab-pane :label="$t('strategy.plan.tab.tradeInfo')" name="tradeInfo">
          <tradeInfo ref="hedgeDialog" :strategyCode="formData.strategyCode" :strgEquation="formData.strgEquation" :maxShare="formData.maxShare" :strategyNature="formData.strategyNature" :disabled="matchFlag"></tradeInfo>
        </el-tab-pane>
      </el-tabs>
...
<script>
import tradeInfo from '@/views/frame/rm/strategy/plan/spotSingle/tradeInfo'
    ...
    checkSave() {
      for (var i = 0; i < this.formData.detailList.length; i++) {
        ...
        if (!data.direction) {
          this.$notify(
            notifyError({
              msg: this.$t('strategy.plan.msg.spotDirectionIsEmpty')
            })
          )
          return true
        }
