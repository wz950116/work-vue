    ...
    <el-row :gutter="20" style="border-top:1px solid #aaaaaa;">
      <el-col :span="12" style="border-right:1px solid #aaaaaa;">
        ...
        <el-table stripe border height="150" highlight-current-row ref="buyMultipleTable" class="table-content tb-edit" :data="buyList" style="border:1px solid;" @current-change="buyListChange">
          ...
          <el-table-column :width="90" align="center" :label="$t('strategy.plan.list.itemClass')" prop="itemClass">
          ...
          <el-table-column :width="120" :label="$t('strategy.plan.list.productCode')" prop="productCode">
            <template slot-scope="scope">
              <base-select v-if="scope.row.itemClass === '1'" v-model="scope.row.productCode" :attrs="{data: 'DD_PRODUCT', disabled:true ,filterable: true}" size="mini"></base-select>
              <base-select v-else v-model="scope.row.productCode" :attrs="{data: 'FUTURES_VARIETIES', disabled:true ,filterable: true}" size="mini"></base-select>
            </template>
          </el-table-column>
          <el-table-column :width="120" :label="$t('strategy.plan.list.contractCode')" prop="contractCode">
            ...
            <template slot-scope="scope">
              <base-select v-model="scope.row.contractCode" :attrs="{data: 'FUTURES_CONTRACT',  disabled:scope.row.itemClass === '1' || disabled ,params: {futuresVarietiesCode: scope.row.productCode} ,filterable: true}" size="mini"></base-select>
            </template>
          </el-table-column>
          <el-table-column :width="80" :label="$t('strategy.plan.list.offset')" prop="offsetFlag">
            ...
            <template slot-scope="scope">
              <el-select v-model="scope.row.offsetFlag" :clearable="true" :disabled="scope.row.itemClass!=='2' || disabled" :placeholder=" $t('biz.placeholder.choose')" size="mini">
          ...
          <el-table-column :width="250" :label="$t('strategy.plan.list.priceLimit')">
            ...
            <template slot-scope="scope">
              <el-row :gutter="0" style="width:100%;">
                <el-col :span="12">
                  <input-formatter :min="0" :max="999999999999.99" type="thousands" :precision="2" v-model="scope.row.minPrice" size="mini" :disabled="priceDisabled || disabled"></input-formatter>
                </el-col>
                <el-col :span="12">
                  <input-formatter :min="scope.row.minPrice || 0" :max="999999999999.99" type="thousands" :precision="2" v-model="scope.row.maxPrice" size="mini" :disabled="priceDisabled || disabled"></input-formatter>
                </el-col>
              </el-row>
            </template>
          </el-table-column>
          <el-table-column :width="250" :label="$t('strategy.plan.list.quantityLimit')">
            ...
            <template slot-scope="scope">
              <el-row :gutter="0" style="width:100%;">
                <el-col :span="12">
                  <input-formatter :min="0" :max="999999999999.99" type="thousands" :precision="2" v-model="scope.row.minQuantity" size="mini"></input-formatter>
                </el-col>
                <el-col :span="12">
          ...
          <el-table-column :width="90" :label="$t('strategy.plan.list.currencyCode')" prop="currencyCode">
            <template slot-scope="scope">
              <base-select v-model="scope.row.currencyCode" :disabled="disabled" :attrs="{data: 'CURRENCY',disabled:disabled}" size="mini"></base-select>
            </template>
          </el-table-column>
          <el-table-column width="120" :label="item.columnTxt" :key="item.id" v-for="item in productItemInfo">
            <template slot-scope="scope">{{scope.row[item.columnName]}}</template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    buyDelete() {
      ...
      if (this.buyCurrentRow.matchQuantity && this.buyCurrentRow.matchQuantity > 0) {
        this.$notify(
          notifyInfo({
            msg: this.$t('strategy.plan.msg.detailIsMatch')
          })
        )
        return
      }
    ...
    sellDelete() {
      ...
      if (this.sellCurrentRow.matchQuantity && this.sellCurrentRow.matchQuantity > 0) {
        this.$notify(
          notifyInfo({
            msg: this.$t('strategy.plan.msg.detailIsMatch')
          })
    ...
    ratioChange(val, row) {
      if (row.floatRatio > 0) {
        row.maxQuantity = this.maxShare * row.quantity * (1 + row.floatRatio / 100)
        row.minQuantity = this.maxShare * row.quantity
      } else {
        row.maxQuantity = this.maxShare * row.quantity
        row.minQuantity = this.maxShare * row.quantity * (1 - row.floatRatio / 100)
      }
    },
    setList(data) {
      data.forEach(e => {
