  ...
  <div class='app-container edit-page-style'>
    ...
    <header :style="{'border-bottom-width': mainInfoVisible ? '1px' : '0'}">
      ...
      <el-form v-if='mainInfoVisible' :inline="true" :model="formData" class="header-form-inline" label-position="left" :rules='rules' ref="validateForm">
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item :label="$t( 'spotProj.fund.edit.originalCurrencyCode')" prop='originalCurrencyCode'>
          ...
          <el-col :span="6">
            <el-form-item :label="$t( 'spotProj.fund.edit.supplierCode')" prop='customerCode'>
          ...
          <el-col :span="6">
            <el-form-item :label="$t( 'spotProj.fund.edit.remark')" prop='remark'>
    ...
    <main>
      <el-tabs v-model="activeName" type="border-card">
        <el-tab-pane :label="$t( 'spotProj.fund.edit.cliamMoney')" name="cliamMoney">
          <fundInfo ref="fundInfo" :direction="formData.purchaseSaleType"></fundInfo>
        </el-tab-pane>
      </el-tabs>
    </main>
...
import Vue from 'vue'
import fundInfo from '@/views/proj/spot/fund/components/fundInfo'
  ...
  methods: {
    ...
    getData() {
      request({
        ...
        setTimeout(() => {
          this.$refs.fundInfo.tableData = response.data.fundDetailDtoList
        }, 500)
      })
    },
    close() {
    ...
    checkSave() {
      var fundInfo = this.$refs.fundInfo.tableData
      if (fundInfo.length === 0) {
        this.$notify(
          notifyInfo({
            msg: this.$t('spotProj.fund.edit.msg.costTypeList')
          })
        )
        return false
      }
      ...
      for (var i = 0; i < fundInfo.length; i++) {
        ...
        if (!data.fundType) {
          this.$notify(
            notifyInfo({
              msg: this.$t('spotProj.fund.edit.msg.fundTypeEmpty')
            })
          )
          return false
        }
        if (!data.spotExposureCode) {
          this.$notify(
            notifyInfo({
              msg: this.$t('spotProj.fund.edit.msg.exposureTypeEmpty')
            })
          )
          return false
        }
        if (parseFloat(data.claimAmt) === 0) {
          this.$notify(
            notifyInfo({
              msg: this.$t('spotProj.fund.edit.msg.claimAmtZero')
            })
          )
          return false
        }
        ...
        if (valid) {
          ...
          this.$confirm(this.$t('biz.msg.confirmSaveAndSubmit'), this.$t('biz.msg.tip'), {
            ...
            .then(() => {
              ...
              this.formData.fundDetailDtoList = this.$refs.fundInfo.tableData
              request({
                url: '/api/spot/fund/' + url,
                method: 'POST',
                data: {
                  funcModule: this.$t('route.' + this.$route.meta.title),
                  funcOperation: this.$t('biz.btn.save'),
    ...
    rateChange() {
      ...
      this.$refs.fundInfo.setAmount()
    },
    typeChange() {
      this.$refs.fundInfo.tableData = []
    }
  }
}
</script>
