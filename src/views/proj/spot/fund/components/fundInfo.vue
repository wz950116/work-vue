<template>
  <div>
    ...
    <el-table stripe border height="150" highlight-current-row ref="buyMultipleTable" class='table-content tb-edit' :data="tableData" style="border:1px solid;" @current-change="tableDataChange">
      ...
      <!-- 费用类型 -->
      <el-table-column prop='fundType' :label="$t('spotProj.fund.edit.fundType')" width='150'>
        <template slot="header" slot-scope="scope">
          {{$t('spotProj.fund.edit.fundType')}}
        ...
        <template slot-scope="scope">
          <el-select size='mini' v-model="scope.row.fundType" :placeholder="$t('biz.placeholder.choose')">
            <el-option v-for="item in $t('datadict.spotFundType')" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </template>
      </el-table-column>
      <!-- 现货敞口编号 -->
      <el-table-column prop='spotExposureCode' :label="$t('spotProj.fund.edit.spotExposureCode')" width='150'>
        <template slot="header" slot-scope="scope">
          {{$t('spotProj.fund.edit.spotExposureCode')}}
          <span style="color:#f56c6c;" :data-header='scope'>*</span>
        </template>
        <template slot-scope="scope">
      ...
      <!-- 认领金额 -->
      <el-table-column :label="$t('spotProj.fund.edit.cliamAmt')" width='150'>
        <template slot="header" slot-scope="scope">
          {{$t('spotProj.fund.edit.cliamAmt')}}
          <span style="color:#f56c6c;" :data-header='scope'>*</span>
        </template>
        <template slot-scope="scope">
      ...
      <!-- 备注 -->
      <el-table-column :label="$t('spotProj.fund.edit.detailRemark')" width='250'>
        <template slot-scope="scope">
