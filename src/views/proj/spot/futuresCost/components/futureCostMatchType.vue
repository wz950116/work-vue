<template>
  <div v-el-drag-dialog class='dialog-wrapper'>
    <div class='dialog-container' type='mini' :style="{width: this.sidebar.opened ? '250px' : '400px'}">
      <TitleContain :titleName='$t("strategy.strategyManagement.form.type")' @TitleFun="closeTitleDialog" />
      <main class="strategyNature">
        <el-radio-group v-model="natureValue">
          <el-radio v-for="item in $t('datadict.futureCostMatchType')" :key="item.value" :label="item.value">{{item.label}}</el-radio>
        </el-radio-group>
      </main>
      <!-- 最后按钮 -->
