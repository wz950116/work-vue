<template>
  <div>
    ...
    <div style="background: white; overflow: hidden;padding-bottom: 20px;">
      <el-pagination background :small="true" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="page.current" :page-sizes="page.limits" :page-size="page.limit" layout="sizes,total,prev,pager,next,jumper" :total="page.total" v-if="table.hidePage !== true"></el-pagination>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = true
      this.tableData = []
      return this.$http[method](this.table.url, deleteEmptyKey(config))
        .then(res => {
